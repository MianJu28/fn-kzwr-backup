...
impl RestoreJob {
    ...
    pub async fn run(&self, rel_files: &[String], restore_dir: &Path) -> Result<RestoreSummary> {
        ...
        for rel in &files_to_restore {
            ...
            let n = self.restore_one(rel, restore_dir, cb).await?;
            // 下载完成后才计入完成数
            restored += 1;
            restored_bytes += n;
            self.publish(
