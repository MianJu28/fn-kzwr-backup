...
- **插件化扩展（ADR-013）**：备份目标与增强能力都是**插件**（内置：`webdav` 目标、`kzwr` 增强）；另支持**外置插件（动态库 `*.so`，默认关闭）**——把插件放进 `$TRIM_PKGETC/plugins/` 并在设置页开启即可扩展新卡片/新接口/新备份目标，**无需重新打包主程序与前端**。宿主对插件做 ABI 与「编译期宿主版本」双重校验，单个插件失败不影响核心（详见 `docs/ARCHITECTURE.md` ADR-013）
